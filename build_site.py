...
def featured_sets(sets_df, matched, limit):
    preferred = ["Speed Champions","Technic","Architecture","Star Wars","Ideas","Icons","Animal Crossing","Creator","Botanicals","Marvel Super Heroes","City","Pokémon"]
    df = sets_df.copy()
    df["has_video"] = df["slug"].isin(matched)
    df["theme_rank"] = df["Theme"].apply(lambda t: preferred.index(t) if t in preferred else 999)
    df = df.sort_values(["theme_rank","has_video","InstructionsCount","YearFrom"], ascending=[True,False,False,False])
    picks=[]; seen=set()
...
def write_home(config, sets_df, matched, latest_items):
    ...
    body = f"""
<main>
<section class="hero"><div class="container"><div class="hero-card">{f'<div class="hero-banner"><img src="{banner}" alt="Channel banner"></div>' if banner else ''}<div class="hero-content">{f'<img class="hero-avatar" src="{logo}" alt="Channel logo">' if logo else ''}<div class="hero-copy"><p class="hero-mini">Search by set number, theme, or name to find official LEGO building instructions and step-by-step video guides from LEGO Instructions For You.</p><div class="hero-actions"><a class="btn primary" href="#featured-sets">Featured sets</a><a class="btn" href="#latest-videos">Latest videos</a><a class="btn red" href="/themes/">Browse themes</a></div></div></div></div></div></section>
